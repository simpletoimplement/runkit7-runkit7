...
HELP;
		...
		if ($test_cnt) {
			...
			if (getenv('REPORT_EXIT_STATUS') !== '0' &&
				getenv('REPORT_EXIT_STATUS') !== 'no' && ($sum_results['FAILED'] || $sum_results['BORKED'] || ($sum_results['LEAKED'] && !getenv('DONT_FAIL_IF_LEAKED')))) {
				exit(1);
			}
	...
	junit_save_xml();
	if (getenv('REPORT_EXIT_STATUS') !== '0' &&
		getenv('REPORT_EXIT_STATUS') !== 'no' && ($sum_results['FAILED'] || ($sum_results['LEAKED'] && !getenv('DONT_FAIL_IF_LEAKED')))) {
		exit(1);
	}
	exit(0);
}
